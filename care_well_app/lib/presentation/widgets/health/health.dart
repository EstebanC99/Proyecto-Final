// Barrel del módulo de widgets de Mi salud.
export 'health_metric_card.dart';
export 'health_record_highlight_card.dart';
export 'health_event_card.dart';
export 'health_event_type_chip.dart';
export 'health_timeline_view.dart';
export 'health_timeline_tile.dart';
export 'health_timeline_style.dart';
export 'health_disclaimer_banner.dart';
export 'mood_scale.dart';
export 'mood_scale_selector.dart';
export 'mood_bar_chart.dart';
export 'note_card.dart';
export 'habito_realizacion_sheet.dart';
export 'habito_card.dart';
export 'habits_day_progress_header.dart';
export 'tipo_habito_theme.dart';
export 'ultimo_registro_format.dart';
export 'timeline_categorias.dart';
export 'timeline_grouping.dart';
export 'day_group_header.dart';
export 'timeline_category_filter.dart';
// Ficha de salud (US-35)
export 'ficha_salud_list_type.dart';
export 'persona_context_header_card.dart';
export 'blood_type_chip_grid.dart';
export 'status_chip.dart';
export 'health_list_item_card.dart';
export 'health_list_section.dart';
export 'item_form_bottom_sheet.dart';
// Observaciones de bienestar (US-36)
export 'wellbeing_observation_row.dart';
export 'wellbeing_observations_banner.dart';
// Tira de semana + encabezado de día, compartidos con la agenda.
export '../shared/day_header.dart';
export '../shared/week_strip.dart';
