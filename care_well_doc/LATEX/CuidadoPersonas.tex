...
% Tamaño del Sistema
    ...
    \begin{itemize}
        \item \textbf{Alta de responsables:} Facilita el registro de nuevos responsables en el equipo. Esta acción es independiente del alta de cuidadores y asigna explícitamente el rol \textit{Responsable}, otorgando por defecto permisos amplios (editar datos de la persona, gestionar agenda y gestionar equipo), ajustables posteriormente.
        \item \textbf{Baja de responsables:} Permite eliminar a un responsable del equipo de cuidado. Esta acción revocará su acceso y permisos en el sistema.
    ...
    \par En el MVP, los eventos se crean con un tipo genérico y el formulario de alta no expone la selección del tipo de evento de agenda; la clasificación por tipo queda reservada para iteraciones posteriores.
    ...
    \par La gestión de la agenda sobre una persona a cargo está condicionada por los permisos del equipo de cuidado; no obstante, el propio usuario siempre puede registrar sobre su propio perfil sin requerir pertenecer a un equipo de cuidado. En consecuencia, cuando el usuario visualiza su propio contexto puede crear eventos en su propia agenda sin restricción de permisos.
    \subsection{Chats}
    ...
    \begin{itemize}
        ...
        \item \textbf{Baja y reactivación de asignaciones (capa demo):} el comportamiento definido es la baja lógica de la asignación (estado \textit{inactiva}) con una ventana de reactivación de 30 días validada por el backend (véase \textit{Reglas de negocio transversales}). En la capa demo la ventana temporal y la expiración de los 30 días se simulan en el dispositivo; la imposición real del plazo requiere la integración con el backend.
        \item \textbf{Tipo de evento de agenda:} los eventos de agenda se crean con un tipo genérico; el formulario de alta no expone la selección del tipo de evento en el MVP.
