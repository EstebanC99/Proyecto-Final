...
% Análisis de la organización
    ...
    \par\noindent \includegraphics[width=1\textwidth]{Imagenes/Organigrama.jpg}
...
% Análisis de Factibilidad
    ...
    \par La decisión central es \textbf{delegar la ejecución de los modelos en un proveedor de inteligencia artificial gestionado}: se emplea \textbf{Google Gemini} a través de \textbf{Vertex AI}, la plataforma de inteligencia artificial para clientes empresariales de Google Cloud. Esta definición reemplaza al esquema previsto originalmente, que contemplaba ejecutar los modelos sobre infraestructura propia (\textit{self-hosted}) mediante \textit{Ollama}. Dicho esquema debió descartarse durante la puesta en producción, al constatarse que el servidor privado virtual contratado \textbf{no dispone de memoria suficiente} para sostener de manera simultánea el motor de base de datos y un modelo de lenguaje cargado en memoria, y que la latencia de un modelo ejecutado sobre CPU compartida resultaba incompatible con un uso interactivo. Los motivos que sustentan la elección adoptada son los siguientes:
    \begin{itemize}
        \item \textbf{Viabilidad sobre la infraestructura disponible:} es la única alternativa que permite ofrecer ambas funcionalidades sin escalar el servidor ni incorporar hardware especializado. Sostener los modelos en el propio servidor habría exigido ampliar la memoria y, para alcanzar tiempos de respuesta aceptables, contratar un equipo con placa de video dedicada, de costo sensiblemente mayor y fijo con independencia del uso.
        \item \textbf{Calidad y tiempo de respuesta:} se accede a un modelo comercial de primer nivel, con una capacidad de lectura de documentos y de redacción en lenguaje natural muy superior a la de los modelos pequeños que podían alojarse localmente, y con respuestas en el orden de los segundos.
        \item \textbf{Garantías de privacidad del proveedor:} conforme la documentación oficial de Google Cloud, \textbf{los datos enviados a Vertex AI —consignas, imágenes y respuestas generadas— no se utilizan para entrenar sus modelos}, y su tratamiento se rige por un acuerdo formal de tratamiento de datos, el \textit{Cloud Data Processing Addendum} (CDPA)\cite{GoogleCloudDataGovernance}. Estas garantías son propias de la plataforma empresarial y no del servicio gratuito de uso general del mismo proveedor, razón por la cual se optó deliberadamente por la primera.
        \item \textbf{Costo acotado para el volumen previsto:} si bien el servicio se factura por volumen de uso, el proveedor ofrece un nivel gratuito mensual y un precio por consulta reducido. Para el volumen esperado en esta etapa —validaciones de identidad en el alta de cuentas y resúmenes diarios— el costo resulta marginal frente al del resto del proyecto.
    \end{itemize}
    \par Se emplea un \textbf{único modelo} para ambos casos de uso: \textit{gemini-2.5-flash-lite}, una variante liviana y de bajo costo dentro de la familia Gemini. Por tratarse de un modelo \textbf{multimodal}, interpreta tanto texto como imágenes, de modo que resuelve indistintamente la lectura de la fotografía del documento de identidad y la redacción del resumen diario. Unificar el modelo simplifica la configuración, el monitoreo y la estimación de costos del componente respecto del esquema anterior, que requería un modelo de visión y otro de texto por separado.
    \par La solución productiva es la ya descripta y no admite variantes: \textit{Google Gemini} sobre \textbf{Vertex AI}, con las garantías de tratamiento que otorga el \textit{Cloud Data Processing Addendum}. El componente permite, no obstante, \textbf{configurar el proveedor según el ambiente}: durante el \textbf{desarrollo} se emplea el \textbf{nivel gratuito} del mismo proveedor —que expone el mismo modelo y el mismo protocolo de acceso, diferenciándose únicamente en la plataforma que lo publica— y ello se hace \textbf{exclusivamente con datos ficticios}. La restricción es deliberada y responde a que dicho servicio gratuito \textbf{no ofrece las garantías de no utilización de los datos para el entrenamiento de los modelos}, propias de la plataforma empresarial: por ese motivo no se lo considera admisible para información real de personas y, con mayor razón, para datos de salud. En consecuencia, \textbf{la totalidad de los datos reales se procesa únicamente sobre Vertex AI}.
    \par El riesgo asumido pasa a ser la \textbf{dependencia de un tercero}: cambios de precios o de condiciones de servicio, discontinuación de la versión del modelo empleada y remisión de información sensible fuera de la infraestructura propia. Dicho riesgo se considera aceptable y se acota por tres vías: las garantías contractuales del proveedor ya señaladas; la \textbf{minimización de los datos} efectivamente enviados, detallada en las subsecciones \textit{Validación de identidad} y \textit{Resumen diario de la persona a cargo}; y el hecho de que la integración se encuentra \textbf{encapsulada en un componente propio}, lo que permite sustituir el proveedor o el modelo sin afectar al resto del sistema. A ello se suma que ninguna de las dos funcionalidades toma decisiones críticas de forma autónoma: la validación de identidad se limita a comparar datos y, ante la duda, rechaza y solicita un nuevo intento; el resumen es una ayuda de lectura que no reemplaza a las pantallas de detalle. El tratamiento de este riesgo se desarrolla en la sección \textit{Riesgo 2 - Integración de IA}.
    ...
    \par En particular, el Usuario reconoce y acepta que, para prestar las funcionalidades de \textit{validación de identidad} y de \textit{resumen diario}, la aplicación procesa datos personales —incluidos la imagen del documento de identidad y datos de salud— mediante servicios de \textbf{inteligencia artificial de un proveedor externo}, que actúa como encargado del tratamiento y no emplea dicha información para entrenar sus modelos, en las condiciones detalladas en la \textit{Política de Privacidad}.
    \saltolinea
    ...
    \end{itemize}
    \par Mención aparte merece la \textbf{imagen del documento de identidad} que se solicita al crear una cuenta. Dicha imagen se emplea \textbf{únicamente} para comprobar que los datos declarados corresponden al titular del documento y \textbf{no se almacena}: se procesa en el momento y se descarta, conservándose solamente la constancia de que la identidad fue validada y la fecha en que ello ocurrió (véase la subsección \textit{Validación de identidad}). Para realizar esa comprobación, la imagen se remite a un \textbf{proveedor externo de inteligencia artificial} —Google Cloud, a través de su plataforma empresarial \textit{Vertex AI}—, que interviene como encargado del tratamiento, la procesa en el momento y no la conserva.
    \par Del mismo modo, la funcionalidad de \textbf{resumen diario} entrega a ese mismo proveedor el nombre de la persona a cargo junto con los eventos de salud, hábitos de vida y estados de ánimo del día, con la \textbf{única finalidad} de redactar el texto que se muestra al usuario. Se envía solamente la información necesaria para esa redacción y no se remite el historial completo de la persona ni identificadores adicionales. El texto así redactado \textbf{se conserva en nuestros sistemas} para poder mostrarlo nuevamente durante un lapso breve sin volver a generarlo; se guarda \textbf{únicamente el resumen más reciente} de cada persona —cada nueva generación reemplaza al anterior, de modo que no se construye un historial— y se \textbf{elimina junto con los datos de la persona} cuando esta se da de baja. Al utilizar la aplicación, el Usuario \textbf{presta su consentimiento expreso} para este tratamiento, en los términos de la presente política y de los \textit{Términos y condiciones}.
    \par Respecto de ambos usos, y conforme la documentación oficial del proveedor, las consignas enviadas —incluidas las imágenes— y las respuestas generadas \textbf{no se utilizan para entrenar sus modelos}, y su tratamiento se rige por un acuerdo formal de tratamiento de datos, el \textit{Cloud Data Processing Addendum}\cite{GoogleCloudDataGovernance}. Estas garantías corresponden a la plataforma empresarial contratada y no al servicio gratuito de uso general del mismo proveedor, motivo por el cual se optó deliberadamente por aquella. Los datos así remitidos viajan por canales cifrados y no se ceden a ningún otro tercero.
    ...
    \end{itemize}
    \par Esta política se encuentra \textbf{disponible dentro de la propia aplicación}: puede consultarse en cualquier momento desde la pantalla de \textit{Configuración}, en el grupo \textit{Legal}, junto con los \textit{Términos y condiciones} (véase la subsección \textit{Configuración}). El texto que allí se muestra es la transcripción del aquí expuesto, incluido el listado precedente de derechos reconocidos por la Ley N° 25.326. De este modo se satisface tanto la remisión que los \textit{Términos y condiciones} hacen a este documento como la obligación legal de \textbf{exhibir de manera clara y visible} los derechos que asisten al titular de los datos.
    ...
    \par Dado que para los servicios de hosting y aplicaciones para el desarrollo ya contamos con licencias, no se consideran, por el momento, costos adicionales de materiales. Se evaluará a futuro en caso de necesitar aplicar algún recargo del mismo.
    \par Corresponde señalar, no obstante, una partida de servicios de carácter \textbf{variable}: el consumo de la \textbf{API de inteligencia artificial} (\textit{Google Gemini} sobre \textit{Vertex AI}), que se factura por volumen de uso y no de forma fija (véase la subsección \textit{Componente de inteligencia artificial}). Para el volumen previsto en esta etapa —acotado a las validaciones de identidad del alta de cuentas y a los resúmenes diarios— el consumo se mantiene dentro del nivel gratuito mensual que ofrece el proveedor o, superado este, en un monto marginal frente al resto del presupuesto, por lo que no se incorpora como partida al cálculo total. Se prevé su seguimiento y su reevaluación en el caso de que el crecimiento de la base de usuarios lo vuelva significativo.
    \subsubsection{Total del presupuesto}
    \par Tomando en cuenta los cálculos de costos de personal y el carácter marginal, en esta etapa, de los costos de materiales y servicios señalados, el costo total de presupuesto se traduce en el siguiente:
...
% Análisis de Factibilidad
    ...
    \par\noindent \includegraphics[width=1\textwidth]{Imagenes/DER.png}
...
% Metodología a Utilizar
    ...
    \saltolinea
    \par\noindent \includegraphics[width=1\textwidth]{Imagenes/IterativoIncremental.png}
...
% Tamaño del Sistema
    ...
    \begin{itemize}
        ...
        \item \textbf{Modificación de datos:} Actualizar la información personal del usuario, tal como sus nombres o sus datos de contacto, desde la pantalla de \textit{Mi Perfil}, en la que cada dato se edita en el mismo lugar donde se lo consulta y con los mismos criterios de validación que rigen en el alta. El \textbf{rol} de una persona no es un atributo del usuario sino de cada asignación de cuidado, y no admite modificación una vez creada (véase la subsección \textit{Reglas de negocio transversales}).
    \end{itemize}
    ...
    \end{itemize}
    \par La lectura de la imagen la realiza un componente de inteligencia artificial provisto por un \textbf{proveedor externo} —\textit{Google Gemini} sobre la plataforma empresarial \textit{Vertex AI} de Google Cloud—, al que la fotografía se remite \textbf{exclusivamente} para extraer los tres datos indicados. El proveedor actúa como encargado del tratamiento: no conserva la imagen ni la utiliza para entrenar sus modelos, y su intervención se rige por un acuerdo formal de tratamiento de datos\cite{GoogleCloudDataGovernance}. El envío se realiza sobre canales cifrados y se limita a la imagen aportada, sin acompañarla de otros datos de la persona. Este criterio es el mismo adoptado para el \textit{Resumen diario} (véanse las subsecciones \textit{Componente de inteligencia artificial}, \textit{Resumen diario de la persona a cargo} y \textit{Riesgo 2 - Integración de IA}).
    ...
    \par En la parte superior, el menú principal presenta un \textbf{encabezado} con el saludo personalizado al usuario y un \textbf{acceso a la Configuración}, representado por la imagen de perfil del usuario. Desde la pantalla de \textit{Configuración} se accede, a su vez, a \textit{Mi Perfil} —a través de la tarjeta de identidad que la encabeza—, donde el usuario visualiza y modifica sus datos personales (véanse las secciones \textit{Mi Perfil} y \textit{Configuración}). Todas las pantallas secundarias cuentan con un botón que permite regresar fácilmente al menú principal.
    \par Debajo del encabezado, la pantalla presenta el \textbf{selector de persona a cargo} y una \textbf{tarjeta destacada con el Resumen diario} de la persona de contexto, que muestra una versión acotada del texto redactado por el componente de inteligencia artificial junto con el \textbf{momento en que fue generado}, y desde la cual se accede al resumen completo (véase la sección \textit{Resumen diario de la persona a cargo}). La tarjeta se resuelve de forma independiente del resto de la pantalla: mientras el resumen se obtiene, o si su generación falla, el menú principal permanece plenamente operativo.
    ...
    \par El módulo \textit{Mi perfil} permite al usuario \textbf{visualizar y editar su propia información personal} en una \textbf{única pantalla}. Se descartó el esquema anterior, que separaba una pantalla de consulta de un formulario de edición independiente: la modificación ocurre \textbf{en el mismo lugar donde se lee el dato}, de modo que el usuario no abandona la pantalla ni debe recordar en qué formulario se edita cada campo. Se ingresa desde la tarjeta de identidad ubicada en la parte superior de \textit{Configuración}, y la pantalla ofrece un botón de navegación para regresar.
    \par La pantalla se organiza en tres bloques:
    \begin{itemize}
        \item \textbf{Encabezado:} presenta la \textbf{fotografía} del usuario —que puede ampliarse o reemplazarse desde el propio avatar—, su nombre completo y su correo electrónico, acompañados de un \textbf{distintivo con el rol} que la persona ejerce en el sistema (\textit{Responsable} o \textit{Cuidador}). El rol se muestra como atributo informativo del encabezado y \textbf{no es editable}, en coherencia con la regla que establece que el rol pertenece a cada asignación de cuidado y no al usuario (véase \textit{Reglas de negocio transversales}); se determina a partir de sus asignaciones activas y, cuando el usuario todavía no integra ningún equipo de cuidado, el distintivo no se muestra.
        \item \textbf{Cifras de resumen:} una tarjeta indica la cantidad de \textbf{personas a cargo} del usuario, la cantidad de personas en cuyo cuidado \textbf{colabora} y su \textbf{edad}. Se trata de información \textbf{derivada}, calculada a partir de las asignaciones de cuidado activas y de la fecha de nacimiento; no constituye un dato nuevo que el sistema almacene. Las asignaciones se cuentan por \textbf{persona distinta}, de modo que dos asignaciones sobre una misma persona cuidada computan una sola vez.
        \item \textbf{Datos personales:} agrupados en \textit{Datos de contacto} (correo electrónico y teléfono) e \textit{Identificación} (documento y fecha de nacimiento).
    \end{itemize}
    \par La \textbf{edición} se rige por las siguientes reglas:
    \begin{itemize}
        \item \textbf{Edición campo a campo:} el teléfono, el documento y la fecha de nacimiento se modifican de forma individual. Cada dato ofrece un ícono de edición que convierte la fila en un campo editable y habilita las acciones de confirmar o cancelar; la fecha de nacimiento se elige mediante un selector de calendario que no admite fechas futuras. Cada confirmación persiste únicamente el dato modificado.
        \item \textbf{Nombre y apellido en conjunto:} ambos se editan \textbf{juntos}, en un panel emergente con un único guardado. Se resolvió así de forma deliberada para no admitir un estado intermedio en el que convivan el nombre nuevo y el apellido anterior.
        \item \textbf{Validaciones consistentes con el alta:} los datos editados se validan con los \textbf{mismos criterios que rigen en el registro}. En particular, el \textbf{documento} se valida con el mismo formato exigido al crear la cuenta, de modo que un dato ya contrastado durante la \textit{validación de identidad} no pueda degradarse luego desde el perfil.
        \item \textbf{Correo electrónico de solo lectura:} el correo se exhibe pero no se edita desde esta pantalla, por tratarse de una \textbf{credencial de acceso} y no de un dato de contacto más. Su modificación requiere un flujo de verificación propio, no comprendido en el alcance de esta versión.
        \item \textbf{Confirmación y protección del guardado:} toda modificación exitosa se confirma con un aviso al usuario y, mientras el guardado está en curso, la pantalla impide abandonarla para no dejar la operación a mitad de camino.
    \end{itemize}
    \subsection{Configuración}
    \par El módulo de \textit{Configuración} brinda a los usuarios diversas opciones para gestionar su cuenta y mantener la seguridad de su información. Sus opciones se presentan \textbf{agrupadas por finalidad}, en el siguiente orden: la identidad del usuario, \textit{Seguridad y privacidad}, \textit{Legal} y \textit{Zona sensible}, seguidas del cierre de sesión y de un pie con la versión instalada de la aplicación. La agrupación busca que el usuario distinga con claridad las acciones inocuas de aquellas que comprometen su cuenta.
    \begin{itemize}
        \item \textbf{Identidad del usuario:} en el extremo superior, una tarjeta muestra la fotografía, el nombre y el correo electrónico del usuario autenticado, y constituye el \textbf{acceso a \textit{Mi Perfil}} (véase la subsección \textit{Mi Perfil}). Esta tarjeta reemplaza al ítem de menú homónimo que existía anteriormente: cumple la misma función de acceso y, además, permite al usuario confirmar de un vistazo con qué cuenta está operando.
        \item \textbf{Seguridad y privacidad — Cambiar contraseña:} Permite al usuario con sesión iniciada modificar su contraseña. A diferencia del flujo de \textit{Recuperación de contraseña} —pensado para quien no puede acceder a su cuenta y por ello se apoya en un código enviado por correo—, aquí la identidad del usuario ya está acreditada por la sesión activa, de modo que el sistema solicita la \textbf{contraseña actual} junto con la nueva. La contraseña propuesta debe cumplir las \textbf{mismas exigencias de complejidad que rigen en el alta de la cuenta} y confirmarse en un segundo campo. La contraseña actual, en cambio, se exige únicamente como dato requerido y no se somete a esa validación de complejidad, para no dejar sin posibilidad de cambio a las cuentas creadas con anterioridad a la vigencia de dicha exigencia.
        \item \textbf{Legal — Términos y condiciones:} Permite acceder a una pantalla que muestra los términos y condiciones aceptados al crear la cuenta. Los usuarios pueden revisarlos en cualquier momento y regresar al menú anterior mediante un botón de navegación.
        \item \textbf{Legal — Política de privacidad:} Muestra, en una pantalla equivalente a la anterior, el texto íntegro de la \textit{Política de privacidad} del proyecto, incluido el listado de derechos que la Ley N° 25.326 reconoce a los titulares de los datos personales (véase la subsección \textit{Política de privacidad}). Hasta esta versión la política solo se mencionaba en los \textit{Términos y condiciones} sin ser consultable desde la aplicación; su incorporación como pantalla propia satisface esa remisión y la obligación de exhibir dichos derechos de manera clara y visible.
        \item \textbf{Legal — Acerca de CareWell:} Presenta un cuadro de diálogo informativo con el \textbf{nombre y el isotipo} de la aplicación, la \textbf{versión instalada} en el dispositivo, una breve descripción de su propósito, la \textbf{información de licencias} del software de terceros utilizado y una dirección de \textbf{contacto}. La exhibición de las licencias responde a las obligaciones de atribución de los componentes de código abierto que la aplicación incorpora.
        \item \textbf{Zona sensible — Eliminar cuenta:} Ofrece al titular la opción de dar de baja su propia cuenta. Se presenta separada del resto y con tratamiento visual diferenciado, advirtiendo que el usuario \textbf{perderá el acceso} y que sus datos \textbf{dejarán de estar disponibles}. Antes de proceder se solicita una \textbf{confirmación explícita}, para la cual el usuario debe transcribir una palabra de seguridad; si cancela, no se realiza cambio alguno. Al confirmar, la cuenta pasa a estado \textit{eliminada}: el usuario pierde el acceso al sistema y se \textbf{desactivan todos sus dispositivos registrados}, de modo que deja de recibir notificaciones. Se trata de una \textbf{baja lógica}: el registro de la persona se conserva, ya que puede integrar el equipo de cuidado de terceros y su información forma parte del historial de otras personas del sistema. La supresión definitiva de los datos personales puede solicitarse por los canales previstos en la \textit{Política de privacidad}, conforme a los derechos reconocidos por la normativa de protección de datos.
        \item \textbf{Cerrar sesión:} Permite al usuario cerrar la sesión y regresar automáticamente a la pantalla de inicio, previa confirmación. Se ofrece como acción \textbf{permanentemente visible al pie} de la pantalla y con un tratamiento deliberadamente \textbf{no destructivo}, para diferenciarla de la baja de cuenta: cerrar sesión no compromete la cuenta y el usuario puede volver a ingresar cuando lo desee.
        \item \textbf{Versión de la aplicación:} al pie se indica la \textbf{versión instalada}, dato necesario para el soporte y para que el usuario verifique si dispone de la última publicación.
    \end{itemize}
    \par Las opciones que exponen textos legales o informativos —\textit{Términos y condiciones}, \textit{Política de privacidad} y \textit{Acerca de CareWell}— no requieren conexión con el servidor: su contenido acompaña a la aplicación, de modo que el usuario pueda consultarlo en cualquier circunstancia.
    ...
    \begin{itemize}
        ...
        \item \textbf{Actividades de agenda:} las ocurrencias de eventos de agenda (\textit{EventoAgenda}), incluidas las derivadas de recurrencias, que aún están por transcurrir (véase la sección \textit{Gestión de Agenda}).
    \end{itemize}
    \par Corresponde señalar que la incorporación de la \textit{Agenda} como cuarto origen de datos estaba originalmente \textbf{reservada para una iteración futura} y fue \textbf{adelantada al MVP} durante la implementación, al advertirse que un resumen que sólo mira hacia atrás resulta de escasa utilidad para organizar el cuidado: el cuidador necesita saber, junto con lo que ya ocurrió, qué queda por hacer. Se deja constancia expresa del cambio de alcance por tratarse de un desvío respecto de lo planificado.
    \par El \textbf{alcance temporal} del resumen es acotado y fijo. Para los tres orígenes de registros ya cargados —eventos de salud, hábitos y estados de ánimo— comprende el \textbf{día actual (hoy)}, y el resumen los presenta como información \textbf{ya registrada}. La \textit{Agenda}, en cambio, abarca lo que \textbf{resta del día de hoy} —desde el momento de la generación en adelante— \textbf{y la totalidad del día de mañana}, de modo que el resumen anticipe los compromisos inmediatos; sus ocurrencias se presentan como actividades \textbf{pendientes}, distinción que sólo tiene sentido en este origen por manejar fechas y horas aún no transcurridas.
    \par El \textbf{proceso de generación} sigue estos pasos: el sistema (i) verifica si existe un resumen previamente almacenado que siga vigente para esa persona, en cuyo caso lo devuelve sin más trámite (véase más abajo el apartado sobre almacenamiento y vigencia); (ii) de no haberlo, recopila de forma tradicional los datos crudos de los cuatro orígenes anteriores para la persona de contexto y el rango indicado; (iii) los entrega a un componente de IA generativa mediante una consigna acotada; y (iv) almacena y presenta al usuario el texto narrativo devuelto. El componente de IA \textbf{no toma decisiones de negocio ni accede directamente a la base de datos}: opera únicamente sobre el conjunto de datos que el sistema le provee para esa persona y ese rango.
    \par Las \textbf{reglas de negocio} que rigen el módulo son las siguientes:
    \begin{itemize}
        \item \textbf{Ausencia de datos en un módulo:} si alguno de los cuatro orígenes no tiene información en el rango, el resumen lo refleja de forma explícita (por ejemplo, indicando que no se registraron estados de ánimo en el día) en lugar de omitir la categoría silenciosamente.
        \item \textbf{Ausencia total de datos:} si la persona de contexto no posee ningún dato en ninguno de los cuatro orígenes para el rango, el sistema no invoca al componente de IA y muestra un mensaje informativo indicando que no hay información para resumir en el período. De este modo se evita el costo de una generación sin insumos. Este resultado \textbf{tampoco se almacena}, para que la carga del primer registro del día se refleje de inmediato y no quede congelada la leyenda de ``sin información''.
        \item \textbf{Vigencia del resumen almacenado:} al solicitarse el resumen se reutiliza el último almacenado siempre que haya sido generado \textbf{el mismo día calendario y hace menos de tres horas}; en caso contrario se genera uno nuevo, que reemplaza al anterior.
        \item \textbf{Actualización a pedido:} el usuario puede forzar la regeneración desde la acción \textit{``Actualizar''} de la pantalla del módulo, sin esperar al vencimiento de la vigencia. Para evitar invocaciones repetidas por pulsaciones sucesivas, se aplica un \textbf{piso técnico de un minuto} entre regeneraciones forzadas.
        \item \textbf{Un único resumen por persona:} el sistema conserva a lo sumo un resumen por persona cuidada —el más reciente—, de modo que cada generación sobrescribe a la anterior y no se acumula historial.
    ...
    \par Respecto de \textbf{cuándo se genera y cómo se almacena}, el resumen se produce \textbf{a pedido}, pero su resultado \textbf{se conserva} para ser reutilizado durante un lapso acotado. Cada generación implica una llamada a un modelo de IA de un proveedor externo, con costo por uso y una latencia del orden de los segundos; como la información de un día no varía sustancialmente en el corto plazo, regenerar el texto en cada apertura de la pantalla suponía costo y espera sin valor agregado. En consecuencia, el sistema guarda el último resumen generado para cada persona cuidada y lo reutiliza mientras se mantenga vigente —mismo día calendario y menos de tres horas desde su generación—, dejando siempre disponible la actualización manual. Se almacena \textbf{un único resumen por persona}, el más reciente, de modo que el registro funciona como una \textbf{caché y no como un historial} de información de salud. Esta decisión reemplaza a la definición anterior, según la cual el MVP no incorporaría ningún tipo de cacheo.
    \par Desde el punto de vista del \textbf{modelo de datos}, la persistencia se materializa en la entidad \textit{ResumenDiario}, asociada a la \textit{Persona} cuidada con cardinalidad \textbf{1 a 0..1}, y compuesta por la \textbf{fecha y hora de generación} —que determina la vigencia— y el \textbf{contenido} del resumen. Dos decisiones de modelado merecen justificación:
    \begin{itemize}
        \item \textbf{Contenido almacenado de forma serializada:} el resumen no se descompone en tablas de eventos, hábitos y recomendaciones porque no es información de negocio sino el \textbf{snapshot de una vista derivada}: nunca se consulta por partes ni se edita, y un esquema relacional obligaría a migrar la base cada vez que cambiara la forma de la salida del modelo de IA.
        \item \textbf{Cardinalidad 0..1:} la tabla opera como caché y no como historial. Así se evita acumular indefinidamente textos con información de salud que nadie consulta y no es necesario definir una política de retención. La trazabilidad de las invocaciones al proveedor externo, cuando se requiere, ya está cubierta por el registro de llamadas a servicios externos.
    \end{itemize}
    \par Los \textbf{criterios de aceptación} verificables del módulo, en lo relativo a su generación, son los siguientes:
    \begin{itemize}
        \item Al reabrir la pantalla dentro de las tres horas posteriores a la generación, se muestra el mismo resumen \textbf{sin una nueva invocación} al componente de IA.
        \item Al solicitar \textit{``Actualizar''}, se genera un resumen nuevo que \textbf{reemplaza} al anterior.
        \item El resumen \textbf{indica el momento en que fue generado}, tanto en la tarjeta del \textit{Menú Principal} como en la pantalla completa, de modo que el usuario sepa a qué instante corresponde la información que está leyendo.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Inclusión del nombre de la persona:} se resolvió expresamente que el \textbf{nombre de la persona a cargo se incluya} entre los datos que se entregan al componente de IA para redactar el resumen, ya que permite una redacción natural y personalizada. En este flujo \textbf{no corresponde disociar ni anonimizar} dicho identificador. La decisión se sustenta en dos fundamentos: por un lado, el destinatario del envío no es un tercero cualquiera sino un \textbf{proveedor de nivel empresarial} con garantías contractuales sobre el tratamiento (véase el ítem siguiente); por el otro, el usuario \textbf{acepta expresamente} este uso de sus datos personales al aceptar la \textit{Política de privacidad} y los \textit{Términos y condiciones} de la aplicación. Fuera del nombre y del subconjunto de datos del día, no se remiten identificadores ni información adicional innecesaria.
        \item \textbf{Proveedor de IA y garantías de tratamiento:} el componente de IA generativa se apoya en un \textbf{proveedor externo} —\textit{Google Gemini} sobre la plataforma \textit{Vertex AI} de Google Cloud—, que actúa como \textbf{encargado del tratamiento} de los datos remitidos (véase la subsección \textit{Componente de inteligencia artificial}). La condición exigida para admitir esa figura se encuentra satisfecha: conforme la documentación oficial del proveedor, las consignas enviadas y las respuestas generadas \textbf{no se emplean para entrenar sus modelos}, y el tratamiento se rige por un acuerdo formal, el \textit{Cloud Data Processing Addendum}\cite{GoogleCloudDataGovernance}. Dichas garantías son propias de la plataforma empresarial contratada y no del servicio gratuito de uso general del mismo proveedor, lo que motivó la elección de aquella.
        \item \textbf{Almacenamiento del texto generado:} corresponde señalar con precisión que el resumen redactado por el componente de IA —que contiene datos de salud de la persona cuidada expresados en lenguaje natural— \textbf{se almacena en la base de datos del sistema}, además de remitirse al proveedor externo para su generación. Se trata de un cambio material respecto de lo definido originalmente, cuando el texto se descartaba tras mostrarse. Para preservar el criterio de \textbf{minimización de datos}, el alcance de ese almacenamiento se acota de dos maneras: se conserva \textbf{únicamente el último resumen} de cada persona, a modo de caché, sin construir un historial de textos sobre su salud; y el registro \textbf{se elimina en cascada} junto con la persona, de modo que la supresión de sus datos alcanza también al resumen almacenado. El texto reside en la misma base y bajo los mismos controles de acceso y cifrado que el resto de la información de salud del sistema, y su visualización está sujeta a las reglas de RBAC descriptas más arriba.
    ...
    \end{itemize}
    \par En cuanto al \textbf{modelo empleado}, la decisión se encuentra tomada: el resumen se genera con \textbf{\textit{gemini-2.5-flash-lite}}, una variante liviana y de bajo costo de la familia Gemini, invocada a través de la plataforma \textbf{Vertex AI} de Google Cloud. Es el mismo modelo que utiliza la \textit{Validación de identidad}, lo que unifica la configuración y el monitoreo del componente de IA. Los fundamentos de esta elección se desarrollan en la subsección \textit{Componente de inteligencia artificial}. El presente análisis, por su parte, fija el comportamiento funcional esperado, el alcance del contenido, las reglas de negocio, el control de acceso y los resguardos de privacidad que la implementación debe respetar.
    ...
    \begin{itemize}
        ...
        \item \textbf{Cambio de contraseña desde Configuración:} el comportamiento definido se encuentra especificado y la pantalla ya está disponible en la aplicación, con el pedido de la contraseña actual, su confirmación y la validación de complejidad de la contraseña nueva en los mismos términos que el alta de la cuenta. Lo que resta es el \textbf{servicio del lado del servidor} que efectivice el cambio, aún pendiente de implementación. La \textit{recuperación} de contraseña, en cambio, se encuentra plenamente implementada de extremo a extremo (véase la subsección \textit{Recuperación de contraseña}).
        \item \textbf{Latencia de actualización del Resumen diario:} un registro cargado \textbf{dentro} de la ventana de vigencia de tres horas \textbf{no se refleja} en el resumen mostrado hasta que el usuario lo actualice manualmente o hasta que dicha ventana venza. Es una consecuencia deliberada de la decisión de conservar el último resumen generado (véase \textit{Resumen diario de la persona a cargo}): se privilegió el ahorro de invocaciones pagas a un proveedor externo y la reducción del tiempo de espera por sobre la inmediatez del reflejo, que el usuario puede recuperar en cualquier momento con la acción \textit{``Actualizar''}. Como mejora posterior se contempla \textbf{invalidar automáticamente el resumen almacenado} al detectarse altas o modificaciones en cualquiera de sus orígenes de datos, de modo que la próxima consulta lo regenere sin intervención del usuario.
...
% Análisis de Riesgos
    ...
    \begin{itemize}
        ...
        \item Contratar los servicios de IA únicamente con proveedores que ofrezcan \textbf{garantías contractuales} de no utilizar los datos remitidos para entrenar sus modelos ni para fines distintos de la prestación del servicio, formalizadas en un \textbf{acuerdo de tratamiento de datos} —en el esquema vigente, el \textit{Cloud Data Processing Addendum} de Google Cloud\cite{GoogleCloudDataGovernance}—, y revisar periódicamente la vigencia de dichas condiciones.
        \item Aplicar \textbf{minimización de datos} en cada invocación, remitiendo al proveedor únicamente el subconjunto necesario para la tarea y evitando el envío de historiales completos o de identificadores prescindibles, sobre canales \textbf{cifrados en tránsito}.
        \item Mantener la integración de IA \textbf{encapsulada en un componente propio}, de modo que resulte posible sustituir el proveedor o el modelo —o replegar la funcionalidad a infraestructura propia— sin impacto en el resto del sistema.
    ...
    \par El segundo es el \textbf{Resumen diario}, presente en el menú principal como tarjeta destacada y ampliable en su propia pantalla, que consolida en un texto narrativo los eventos de salud, los hábitos de vida, los estados de ánimo y las actividades de agenda pendientes de la persona de contexto, de modo que el cuidador comprenda de un vistazo la situación de cuidado sin recorrer módulo por módulo. El último resumen generado se conserva y se reutiliza durante un lapso acotado, con posibilidad de actualizarlo a pedido (véase la sección \textit{Resumen diario de la persona a cargo}).
