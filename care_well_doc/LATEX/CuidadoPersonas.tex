...
% Tamaño del Sistema
    ...
    \par Finalmente, para quienes aún no tengan una cuenta, se incluye el botón \textit{"Crear cuenta"}, acompañado del mensaje \textit{"¿Aún no tienes cuenta?"}. Al pulsarlo, el sistema redirigirá a la pantalla de registro. Una vez completado el registro con éxito, el sistema muestra una pantalla de confirmación y el usuario regresa de forma manual a esta pantalla de inicio de sesión para autenticarse; no se realiza un inicio de sesión automático tras el alta.
    ...
    \end{itemize}
    \par Una vez procesado correctamente el alta, el sistema presentará una pantalla de confirmación con una animación y el mensaje \textit{"Cuenta creada exitosamente"}. Dicha pantalla incluye un único botón \textit{"Ir al login"} que conduce a la pantalla de inicio de sesión. El registro no realiza un inicio de sesión automático: el usuario debe ingresar sus credenciales de forma manual desde la pantalla de login para acceder al sistema.
    ...
    \begin{itemize}
        ...
        \item \textbf{Recomendaciones médicas:} Muestra recomendaciones personalizadas derivadas automáticamente por el sistema a partir de la información registrada por el usuario (hábitos de vida y eventos de salud). Es una sección de solo lectura: el usuario no carga, edita ni elimina recomendaciones. Estas recomendaciones no constituyen opinión médica ni diagnóstico.
        \item \textbf{Eventos de salud:} Permite registrar eventos relevantes como hospitalizaciones, consultas médicas y otros acontecimientos significativos para la salud del usuario. El \textit{Evento de Salud} es el registro persistente y enriquecible del historial: además de cargarse manualmente, puede originarse de forma automática a partir de un evento de agenda con la opción \textit{“Generar evento de salud”} activada, una vez que dicho evento efectivamente ocurrió, quedando vinculado al evento de agenda que lo originó y a la fecha de ocurrencia específica, de modo que el usuario pueda añadirle notas posteriormente (véase la sección \textit{Gestión de Agenda}). Por integridad del historial médico, los eventos de salud no se pueden modificar una vez registrados; ante un error, el evento puede eliminarse (con confirmación) y registrarse nuevamente. Para añadir información sin alterar el evento original se utilizan notas.
    \end{itemize}
    ...
    \par Cabe destacar que los eventos de salud son inmutables una vez creados: no admiten modificación, sino únicamente su eliminación con confirmación previa, a fin de preservar la integridad y trazabilidad del historial médico.
