...
> `care_well_app/`), el backend .NET en `care_well_backend/` y la documentación del proyecto
> (`care_well_doc/`). Frontend y backend se desarrollan de forma desacoplada dentro del mismo repo.
...
│   └── lib/ ...
├── care_well_backend/
│   └── CareWell/                # solución .NET 10 — ver sección 4b
│       ├── CareWell.API/
│       ├── CareWell.Domain/
│       ├── CareWell.Repository/
│       ├── CareWell.BusinessService/
...
- **Patrón arquitectónico:** CQRS implementado a mano, sin MediatR ni librerías externas de mediación.
- **Identidad/JWT:** `System.IdentityModel.Tokens.Jwt`.
- **Estructura de proyectos:**
  - `CareWell.API` — entry point, controllers, Program.cs.
  - `CareWell.Domain` — entidades (`BaseEntity`, propiedades `virtual`, métodos de comportamiento),
    Value Objects (`record`), Factories (`IBaseFactory`/`BaseFactory`).
  - `CareWell.Repository` — DbContext (Fluent API, `ApplyConfigurationsFromAssembly`),
    `IRepository<T>`/`Repository<T>`, `IUnitOfWork`/`UnitOfWork`, repositorios específicos.
  - `CareWell.BusinessService.Abstractions` — interfaces de servicios.
  - `CareWell.BusinessService` — implementaciones; base abstracta `BusinessService(IUnitOfWork)`.
  - `CareWell.Commands` — DTOs de escritura (input CQRS); sin dependencias.
...
- `arquitecto-software`: decisiones de arquitectura tanto del frontend Flutter como del backend
  .NET, modelado de dominio, diagramas ER (draw.io) y revisiones de código. Tiene presente el
  modelo de dominio. **No escribe código de producción** (ni Flutter ni .NET).
- `dev-flutter`: implementa features del frontend. Tiene presente el modelo de dominio.
  **Siempre** presenta un plan numerado y espera confirmación antes de codificar.
...
- Backend (en `care_well_backend/CareWell/`): `dotnet build` · `dotnet run --project CareWell.API`
